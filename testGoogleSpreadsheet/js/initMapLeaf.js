		...
		if (type == 'MultiPolygon' && color == '#2c7bb6') {
			...
			for (i=0; i < googleSpreadsheet.length; i++) {
				if (target == googleSpreadsheet[i][0]) {
					...
					popup.setLatLng(e.latlng).setContent("<b>" + target + " County</b>" + "<br>" + "Population 2000: " + googleSpreadsheet[i][1] + "<br>" + "Population 2010: " + googleSpreadsheet[i][2] + "<br>" +
					...
					for (m=0; m < popupCountyArr.length; m++) {
						...
						if (link == 'null') {
							// deactivate link
							document.getElementById(popupCountyArr[m][0]).disabled = true;
						} else {
							// activate link
